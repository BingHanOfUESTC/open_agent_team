...
用于从素材目录或搜索结果下载/复制图片到 `materials/images/raw/`，裁剪 resize 到 `materials/images/processed/`，并生成 `materials/images/image_manifest.json` 供 `deck_spec.json` 引用。
...
用于解析 `.pptx` 的 slide size、主题色、字体、版式、文本框、图片、形状和备注。模板 PPT 只提取风格，不复用无关内容。
